...
class mu_1_MessageType: public mu__byte
{
 public:
  ...
  mu_1_MessageType (char *name, int os): mu__byte(6, 16, 4, name, os) {};
  mu_1_MessageType (void): mu__byte(6, 16, 4) {};
  mu_1_MessageType (int val): mu__byte(6, 16, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1_MessageType::values[] = {"GetS","GetData","GetM","Inv","Inv_Ack","PutM","PutS","Put_Ack","Fwd_GetS","Fwd_GetM","Put_Ack_Back",NULL };
...
class mu_1__type_0: public mu__byte
{
 public:
  ...
  mu_1__type_0 (char *name, int os): mu__byte(17, 25, 4, name, os) {};
  mu_1__type_0 (void): mu__byte(17, 25, 4) {};
  mu_1__type_0 (int val): mu__byte(17, 25, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1__type_0::values[] = {"H_Invalid","H_Modified","H_Shared","HT_S_D","HT_M_D","HT_M_P","HT_M_A","HT_S_P","HT_I_P",NULL };
...
const int mu_P_Invalid = 26;
const int mu_P_Shared = 27;
const int mu_P_Modified = 28;
const int mu_PT_IS_D = 29;
const int mu_PT_IM_AD = 30;
const int mu_PT_IM_A = 31;
const int mu_PT_SM_AD = 32;
const int mu_PT_SM_A = 33;
const int mu_PT_MI_A = 34;
const int mu_PT_SI_A = 35;
const int mu_PT_II_A = 36;
...
/*** Variable declaration ***/
mu_1__type_7 mu_InBox("InBox",1688);

/*** Variable declaration ***/
mu_0_boolean mu_msg_processed("msg_processed",2456);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",2464);
...
const unsigned numrules = 50;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 50
...
/********************
  Invariant records
 ********************/
int mu__invariant_18() // Invariant "values in shared state match memory"
{
bool mu__quant19; 
mu__quant19 = TRUE;
{
...
bool mu__boolexpr20;
bool mu__boolexpr21;
  if (!((mu_HomeNode.mu_state) == (mu_H_Shared))) mu__boolexpr21 = FALSE ;
  else {
  mu__boolexpr21 = ((mu_Procs[mu_n].mu_state) == (mu_P_Shared)) ; 
}
...
bool mu__condition_22() // Condition for Rule "values in shared state match memory"
{
...
int mu__invariant_23() // Invariant "Invalid implies empty sharer list"
{
bool mu__boolexpr24;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr24 = TRUE ;
  else {
...
bool mu__condition_26() // Condition for Rule "Invalid implies empty sharer list"
{
...
int mu__invariant_27() // Invariant "modified implies empty sharers list"
{
bool mu__boolexpr28;
  if (!((mu_HomeNode.mu_state) == (mu_H_Modified))) mu__boolexpr28 = TRUE ;
  else {
...
bool mu__condition_30() // Condition for Rule "modified implies empty sharers list"
{
...
int mu__invariant_31() // Invariant "value is undefined while invalid"
{
bool mu__quant32; 
mu__quant32 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr33;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_Invalid))) mu__boolexpr33 = TRUE ;
  else {
  mu__boolexpr33 = (mu_Procs[mu_n].mu_val.isundefined()) ; 
}
if ( !(mu__boolexpr33) )
...
bool mu__condition_34() // Condition for Rule "value is undefined while invalid"
{
...
int mu__invariant_35() // Invariant "values in modified state match last write"
{
bool mu__quant36; 
mu__quant36 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr37;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_Modified))) mu__boolexpr37 = TRUE ;
  else {
...
bool mu__condition_38() // Condition for Rule "values in modified state match last write"
{
...
{
bool mu__boolexpr40;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr40 = TRUE ;
  else {
...
int mu__invariant_42() // Invariant "value in memory matches value of last write, when shared "
{
bool mu__boolexpr43;
  if (!((mu_HomeNode.mu_state) == (mu_H_Shared))) mu__boolexpr43 = TRUE ;
  else {
...
bool mu__condition_44() // Condition for Rule "value in memory matches value of last write, when shared "
{
...
{
bool mu__boolexpr46;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr46 = TRUE ;
  else {
  mu__boolexpr46 = (mu_HomeNode.mu_owner.isundefined()) ; 
}
...
{"Invalid implies empty owner", &mu__condition_47, NULL, FALSE},
{"value in memory matches value of last write, when shared ", &mu__condition_44, NULL, FALSE},
{"value in memory matches value of last write, when invalid", &mu__condition_41, NULL, FALSE},
{"values in modified state match last write", &mu__condition_38, NULL, FALSE},
{"value is undefined while invalid", &mu__condition_34, NULL, FALSE},
{"modified implies empty sharers list", &mu__condition_30, NULL, FALSE},
{"Invalid implies empty sharer list", &mu__condition_26, NULL, FALSE},
{"values in shared state match memory", &mu__condition_22, NULL, FALSE},
};
