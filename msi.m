...
type
  ...
  HomeState:
    ...
    HT_S_D, HT_M_D, HT_M_P, HT_M_A, HT_S_P, HT_I_P};--transient states during recall
      owner: Node;	
  ...
  ProcState:
    Record
      state: enum { P_Invalid, P_Shared, P_Modified,
                  PT_IS_D, PT_IM_AD, PT_IM_A, PT_SM_AD, PT_SM_A, PT_MI_A, PT_SI_A, PT_II_A
                  };
      val: Value;
      ack: AckType;
    End;
...
case H_Modified:
    ...
    case GetM:
      Send(Fwd_GetM, HomeNode.owner, msg.src, VC1, UNDEFINED, UNDEFINED); --Send Fwd_GetM to owner
        --put "Send Fwd_GetM to "; --put HomeNode.owner;
      HomeNode.owner := msg.src;                           --set owner to Reauest
        HomeNode.state := HT_M_D;


    case PutS:
      Send(Put_Ack, msg.src, HomeType, VC1, UNDEFINED, UNDEFINED);    --send Put-ack to request
        --put "Send Put_Ack to "; --put msg.src;--put "\n";
        HomeNode.state := HT_M_P;


    case PutM:
      Send(Put_Ack, msg.src, HomeType, VC1, UNDEFINED, UNDEFINED);    --Send Put_Ack to Request
        --put "Send Put_Ack to "; --put msg.src;--put "\n";
      if(msg.src = HomeNode.owner)                                    --data from Owner
        then
...
/*    case GetS:
    ...
    case PutS:
      if(IsSharer(msg.src))
      then
      RemoveFromSharersList(msg.src);                    --Remove request from sharers
      Send(Put_Ack, msg.src, HomeType, VC1, UNDEFINED, UNDEFINED);  --send Put-ack to request
        --put "Send Put_Ack to "; --put msg.src;--put "\n";
      else
        msg_processed := false;
...
case PT_SM_AD:
    ...
    case Fwd_GetS:
      msg_processed := false; -- stall message in InBox
        --put "store in box\n";
    case Inv:
      Send(Inv_Ack, msg.src, p, VC0, UNDEFINED, UNDEFINED); --Send Inv_ack to request
        --put "Send Inv_Ack to "; --put msg.src;--put "\n";
      Send(Inv_Ack, HomeType, p, VC0, UNDEFINED, UNDEFINED); --Send Inv_ack to home
        --put "Send Inv_Ack to "; --put HomeType;--put "\n";
      ps := PT_IM_AD;                    --change state to PT_IM_AD
    case GetData:
    pv := msg.val;                                 --get value from message
    LastWrite := pv;
    if(IsMember(msg.src, Proc))
    then
      ps := P_Modified;
